...
public class Main {
	...
	public static void main(String[] args) {
		...
		Plot2DPanel[] plots =
		optimalGroup_kappa_tau(T.T_fun, T.n_axis, T.k_axis);
	...
	private static Plot2DPanel[] optimalGroup_kappa_tau(double[][] Tnk,
		...
		for (double kappa = 0; kappa <= 0.0005; kappa = kappa + 0.000005) {
			int[] pair = dyn.optimalGroup_n_k(Tnk, n_axis, k_axis, kappa,
					0.00001);
			n1[i] = (double) pair[0];
			k1[i] = (double) pair[1];
			kappa_vec[i] = kappa;
		...
		i = 0;
		for (double tau = 0; tau <= 0.00005; tau = tau + 0.000005) {
			int[] pair = dyn.optimalGroup_n_k(Tnk, n_axis, k_axis, 0.0001, tau);
			n2[i] = (double) pair[0];
			k2[i] = (double) pair[1];
			tau_vec[i] = tau;
			i++;
